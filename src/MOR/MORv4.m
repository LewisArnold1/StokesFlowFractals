...
o8pi=1/(8*pi);
cutOff=5;
[P, rSphere, sphereRadius]=getSpherePositions(); %subroutine that returns number and position of spheres


psi=1; % the under/over-relaxation factor.

N=45; % number of boundary nodes
...
while(zeta>1e-6 & zeta<10)
    ...
    for II=1:P
        ...
        ffz=0;
        for IL=1:M
            ffx=ffx+FI(1+(IL-1)*3,II);
            ffy=ffy+FI(2+(IL-1)*3,II);
            ffz=ffz+FI(3+(IL-1)*3,II);
        end
        ...
        for IJ=1:P
            if (II~=IJ)
                 rRel=rSphere(IJ,:)-rSphere(II,:);
                 if (norm(rRel)>cutOff)
                      treatAsSingleForce=true;
                 else
                      treatAsSingleForce=false;
                 end
                 rbD=rb+rRel;
               ...
               %   At=matrixConstructFast(rs,rb+rRel,M,N); 
                ...
                for b=1:N
                    ...
                    if (treatAsSingleForce)
           
                        rbDn2=rbDx*rbDx+rbDy*rbDy+rbDz*rbDz;
                        rbDn=rbDn2^0.5;
                        rbDn3=rbDn*rbDn2;
                        fdotr=ffx*rbDx+ffy*rbDy+ffz*rbDz;
                        RHStemp(p)=RHStemp(p)-o8pi*(ffx/rbDn+rbDx*fdotr/rbDn3);
                        RHStemp(p+1)=RHStemp(p+1)-o8pi*(ffy/rbDn+rbDy*fdotr/rbDn3);
                        RHStemp(p+2)=RHStemp(p+2)-o8pi*(ffz/rbDn+rbDz*fdotr/rbDn3);
                    else
                        for s=1:M
                            q=3*s-2;
...
if (zeta>10)
    ...
    for IJ=1:P
        for II=1:M
            dragIY(IJ)=dragIY(IJ)+FIc(2+(II-1)*3,IJ);
        end
    end
    StokesDrag=6*pi;
end
