...
N=300; % number of boundary nodes
V=[0 0 0]'; % §\eq{\bm{V}}§, particle translational velocity
...
if (zeta>10)
    ...
    for IJ=1:P
        dragtemp=[0 0 0];
        for dragComp = 1:3
                    dragtemp(dragComp)=sum(FIc(dragComp:3:size(FIc,1)+dragComp-3,IJ));
        end
        dragIZ(IJ)=norm(dragtemp);
    end
    StokesDrag=6*pi;
end
